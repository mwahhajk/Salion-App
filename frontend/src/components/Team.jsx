      ...
      <div className="flex flex-wrap gap-6 items-center justify-center">
        {teamData.map((item, index) => (
          <div className="group flex flex-col items-center py-8 text-sm bg-white border border-gray-300/60 w-64 rounded-md cursor-pointer hover:border-primary hover:bg-primary transition">
            <img className="w-24 rounded-full" src={item.image} alt="userImage1" />
            <h2 className="text-gray-700 group-hover:text-white text-lg font-medium mt-2">{item.name}</h2>
            
            <p className="text-center text-gray-500/60 group-hover:text-white/60 w-3/4 mt-4">{item.desc}</p>
